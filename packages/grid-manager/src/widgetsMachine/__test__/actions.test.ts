...
const calculateGridDimensionsSpy: any = jest.spyOn(
  actions,
...
describe('Actions machine module', () => {
  ...
  describe('setWidgetsRules state invoker', () => {
    ...
    it('should log duplicated keys', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Trying to duplicate widget in model ids: 1,2,3,4,2'
      );
    });
  ...
  describe('removeWidgetInCtx state invoker', () => {
    ...
    it('should throw and error if the widget doesnt exit', async () => {
      ...
      try {
        ...
        expect(e.message).toBe(
          `widget trying to be remove doesn't exit in model`
        );
      }
    });
    ...
    it('Should return a new updated model to the widget machine with addons', async () => {
      const context = {
        widgets: {
          ['app-1']: { id: 'app-1', addons: [] },
          ['app-2']: { id: 'app-2', addons: [{ id: 'addon-1' }] }
        },
        renderCycle: {
          widgetsInDom: [{ id: 'app-1' }, { id: 'app-2' }]
        }
      ...
      const result = await removeWidgetInCtxSpy(context, event);
      expect(result).toStrictEqual({
        widgets: { 'app-2': { id: 'app-2', addons: [{ id: 'addon-1' }] } },
        remove: [{ id: 'app-1' }]
      });
    });
  });
  ...
  describe('addAddonsToWidget state invoker', () => {
    ...
    it('should return the updated model and the addon empty if breakpoint isWidgetBreakpointValid', async () => {
      const context = {
        widgets: {
          ['app-1']: {
            id: 'app-1',
